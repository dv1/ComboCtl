...
class ParsedScreenStream(private val displayFrameFlow: SharedFlow<DisplayFrame>) {
    ...
    private var previousParsedScreen: ParsedScreen? = null
     ...
     *
     * There are two special cases in which this will return null.
     *
     ...
     *
     * In both cases, The return value is null to let the caller know that a new screen
     * came in and that it is not recognized. (If in case #1 both frames have the same
     * pixels, the new frame is considered a duplicate, and filtered out.)
     *
     ...
     *         recognized, and parsed. null if a new non-duplicate was received that
     *         could not be recognized/parsed.
     */
    suspend fun getNextParsedScreen(): ParsedScreen? {
        displayFrameFlow.first { displayFrame ->
            if ((previousDisplayFrame == null) || (previousDisplayFrame !== displayFrame)) {
                ...
                // If both previous and current parsed screen are null,
                // then either there is no previous screen (= we just
                // got the first frame) and the current one was not
                // recognized, or neither the previous nor the current
                // display frame were recognized. In both cases, value-
                // compare the frames. If the previous frame is null,
                // the inequality comparison will be true, specifying
                // that this is a new frame. If the previous frame is
                // not null, the previous and next frame are value-
                // compared, pixel by pixel, to check if they have
                // equal contents. If not, isNewFrame is true.
                if ((previousParsedScreen == null) && (parsedScreen == null)) {
                ...
                //
                // 1. previousParsedScreen is null and parsedScreen is not null
                // 2. Neither are null, and the value comparison shows that
                //    their values aren't equal
                //
                // In both cases this means that the current frame is
                // a new, different one.
                if ((previousParsedScreen == null) || (previousParsedScreen != parsedScreen)) {
                    previousDisplayFrame = displayFrame
