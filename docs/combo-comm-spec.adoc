...
section above) must be closed at this point, followed by opening an
insecure RFCOMM connection to the pump.
...
Here, CCM begins to be used in an uncommon way for the Combo.
...
2. Remote terminal (RT) service: This service covers commands for
   controlling the pump in the remote terminal (RT) mode.
   Its ID is 0x48.

3. Command service: This service covers commands for
...
supposed to transmit when finalizing the pairing process, right before
`AL_CTRL_DISCONNECT`. Ruffy does not use it though.#
...
If the pairing process is finished, the client sends
...
be seen on the Combo's 96x32 pixels LCD. The current
display frame is sent in four pieces to the client, over
four `AL_RT_DISPLAY`, each with 96x8 pixels as payload.
The client has to assemble the entire frame out of these
pieces.

The first 2 bytes are the RT sequence, incremented by
the Combo.

The next byte is marked as "reason". Two possible values
are known: 0x48 ("pump") and 0xB7 ("DM").

#TODO: It is unclear at this moment what this exactly means.#

The next byte is the index. Clients use this byte to
check if the piece they just got is associated with
the frame that is currently being assembled or if this
is a piece associated with a new frame. In the latter
case, the client has to discard any existing partial
frame data that may exist. The check is simply to see
if the index has changed since the last `AL_RT_DISPLAY`
command that was received.

The next byte is row, which indicates the portion of
the frame that is updated by this command. Row 0 is
the top row, or in other words, the top 96x8 pixels.
Row 1 is the second row from the top, etc. In terms of
Y coordinates, row 0 covers Y coordinates 0-7, row
1 covers Y coordinates 8-15 etc.

The remaining 96 bytes contain the 96x8 pixels in
this piece. This data is stored as a 1-bit bitmap.
Bit 0 of the first of the 96 bytes is the top left
pixel, or in other words, the pixel at coordinates
(x 0, y 0). Bit 1 of the first byte is the pixel at
the coordinates (x 1, y 0). Bit 7 of the first byte
is the pixel at (x 7, y 0). Bit 0 of the second
byte is the pixel at (x 8, y 0). Bit 0 of byte 12
is the pixel at (x 0 y 1). Bit 1 of byte 12 is the
pixel at (x 1 y 1) etc.
...
for (y = 0; y < 8; y = y + 1)
{
  for (x = 0; x < 12; x = x + 1)
  {
     byte current_byte = payload_pixel_data[x + y*12];
     for (b = 0; b < 8; b = b + 1)
     {
       boolean current_pixel;
       if ((current_byte & (1 << b)) != 0)
         current_pixel = true;
       else
         current_pixel = false;

       output_pixels[b + x*8 + y*96] = current_pixel;
     }
  }
}
```
