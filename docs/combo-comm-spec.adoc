...
If an application layer packet arrives with the reliability bit set,
then the first 2 bytes of its payload contain a 16-bit little
endian unsigned integer that specifies an error code. The value
0 means "no error". If the value is nonzero, this needs to be
interpreted as a non-recoverable error, and the client must
disconnect (by sending the `AL_CTRL_DISCONNECT` command etc).
...
_started_. "Infused" means that it _ended_.
...
* 8: Extended bolus started
* 9: Extended bolus ended
* 10: Multiwave bolus started
* 11: Multiwave bolus ended
...
This administers a standard bolus (that is: neither extended nor multiwave bolus).
The bolus amount is given in 0.1 IU units. For example, to administer 5.7 IU,
the amount "57" is specified.

The bolus amount is added twice to the payload, once as a 16-bit little-endian
integer, once as a 32-bit little-endian floating point.

The payload begins with two bytes 0x55 and 0x59. Their function is not understood,
but without them, the command fails.

Next come the two bytes that make up the 16-bit little-endian integer with the bolus
amount inside.

Two bytes follow (these appear to be related  to multi-wave and extended
bolus types), and again followed by two bytes (related to the multi-wave bolus type).
All four of these bytes currently have to be zero since their exact function is not
understood.

The four bytes of the 32-bit little-endian floating point with the bolus amount inside
follow. Note that even though this is a floating-point value, the amount still is given
in 0.1 IU units here, so in the example above, the floating-point value would still be
57, and _not_ 5.7.

2x4 bytes follow, which fulfill the same function as the 2x2 bytes before (that is:
multi-wave and extended bolus functionality). All of these eight bytes are set to zero.

Finally, 2 bytes are added that contain a 16-bit unsigned little-endian integer with
a CRC-16-MCRF4XX checksum of all payload bytes (except the 2 checksum bytes).
