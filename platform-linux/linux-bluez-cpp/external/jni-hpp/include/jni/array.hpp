...
namespace jni
   {
    ...
    class Array< E, std::enable_if_t<IsPrimitive<E>::value> > : public Object< ArrayTag<E> >
       {
        ...
        public:
            ...
            template < class T >
            void SetRegion(JNIEnv& env, jsize start, jsize len, const T* buf)
               {
                SetArrayRegion(env, SafeDereference(env, this->get()), start, len, buf);
               }
    ...
    template < class T >
    Local<Array<T>> MakeAnything(ThingToMake<Array<T>>, JNIEnv& env, const T* array, std::size_t size)
       {
        Local<Array<T>> result = Local<Array<T>>(env, &NewArray<T>(env, size));
        SetArrayRegion(env, *result, 0, size, array);
        return result;
       }
